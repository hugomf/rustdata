    ...
    D::Id: Clone,
{
    ...
    fn find_by_id_sql() -> String {
        let d = BA::dialect();
        format!("SELECT {} FROM {} WHERE {} = {}", D::select_cols(), D::TABLE, D::id_column().name, d.ph(1))
    }
    ...
    /// Auto-generated find_by_* method
    pub async fn find_by(&self, method_name: &str, values: Vec<SqlValue>) -> Result<Vec<D::Entity>, RepositoryError> {
    ...
    pub async fn find_one_by(&self, method_name: &str, values: Vec<SqlValue>) -> Result<Option<D::Entity>, RepositoryError> {
        let parsed = QueryMethodParser::parse(method_name)?;
        let predicate = QueryMethodParser::build_predicate(parsed, values)?;
    ...
    pub async fn find_all(&self) -> Result<Vec<D::Entity>, RepositoryError> {
        let sql = format!("SELECT {} FROM {}", D::select_cols(), D::TABLE);
        let rows = sqlx::query(&sql).fetch_all(&self.pool).await.map_err(RepositoryError::from)?;
        rows.iter().map(|r| Self::hydrate(r)).collect()
    }

    pub async fn find_all_sorted(&self, sort: &crate::pagination::Sort) -> Result<Vec<D::Entity>, RepositoryError> {
        let sort_clause = Self::build_sort_clause(sort, D::ORDER_BY);
        let sql = format!("SELECT {} FROM {} {}", D::select_cols(), D::TABLE, sort_clause);
        let rows = sqlx::query(&sql).fetch_all(&self.pool).await.map_err(RepositoryError::from)?;
        rows.iter().map(|r| Self::hydrate(r)).collect()
    }

    pub async fn find_all_pred(&self, predicate: &Predicate) -> Result<Vec<D::Entity>, RepositoryError> {
        let (where_clause, params, _) = predicate.to_sql(self.dialect(), 1);
        let sql = if where_clause.is_empty() {
            format!("SELECT {} FROM {}", D::select_cols(), D::TABLE)
        } else {
            format!("SELECT {} FROM {} WHERE {}", D::select_cols(), D::TABLE, where_clause)
        };
        let query = Self::bind_sql_values(sqlx::query(&sql), &params);
        let rows = query.fetch_all(&self.pool).await.map_err(RepositoryError::from)?;
        rows.iter().map(|r| Self::hydrate(r)).collect()
    ...
    pub async fn find_one_pred(&self, predicate: &Predicate) -> Result<Option<D::Entity>, RepositoryError> {
        let (where_clause, params, _) = predicate.to_sql(self.dialect(), 1);
        let sql = if where_clause.is_empty() {
            format!("SELECT {} FROM {}", D::select_cols(), D::TABLE)
        } else {
    ...
    pub async fn insert_batch(&self, entities: Vec<D::Entity>) -> Result<Vec<D::Entity>, RepositoryError> {
    ...
    pub async fn find_one_by_sql(&self, sql: &str, params: &[SqlValue]) -> Result<Option<D::Entity>, RepositoryError> {
    ...
    pub async fn find_many_by_sql(&self, sql: &str, params: &[SqlValue]) -> Result<Vec<D::Entity>, RepositoryError> {
        let rendered = self.dialect().render(sql);
        let query = Self::bind_sql_values(sqlx::query(&rendered), params);
        let rows = query.fetch_all(&self.pool).await.map_err(RepositoryError::from)?;
        rows.iter().map(|r| Self::hydrate(r)).collect()
    ...
    pub async fn execute_sql(&self, sql: &str, params: &[SqlValue]) -> Result<u64, RepositoryError> {
        let rendered = self.dialect().render(sql);
...
SqlValue::Bytes(b) => <AdOf<BA> as BindAdapter<DbOf<BA>>>::bind_bytes(q, b.as_slice()),
             SqlValue::OptStr(s) => <AdOf<BA> as BindAdapter<DbOf<BA>>>::bind_opt_str(q, s.as_ref().map(|s| s.as_str())),
             SqlValue::Null(tid) => match tid {
                SqlTypeId::Uuid => <AdOf<BA> as BindAdapter<DbOf<BA>>>::bind_opt_uuid(q, None),
                SqlTypeId::TimestampTz => <AdOf<BA> as BindAdapter<DbOf<BA>>>::bind_opt_datetime(q, None),
                SqlTypeId::Int | SqlTypeId::BigInt => <AdOf<BA> as BindAdapter<DbOf<BA>>>::bind_opt_int(q, None),
